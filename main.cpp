...
int main(int argc, char** argv) {
    ...
    //////////////////////////////////////////////////////////////////////////// 
    //testts.serial_comm_initialize();
    //testts.gimbal_pitch(0);
    //camobj.control();
    ...
    while (waitKey(1) != 27) {
        ...
        if (!paused) {
         ...
         // draw ellipse
        Size video_size(video_capture.get(CV_CAP_PROP_FRAME_WIDTH), video_capture.get(CV_CAP_PROP_FRAME_HEIGHT));
        int w = video_size.width;
        int h = video_size.height;
        //cout << w << " " << h << endl;
        user_interface->drawMyEllipse(original_frame, Point(w/2,h/2), Size(0.6*w/2,0.5*h/2));
        ...
        if (!paused) {
            ...
            if (object_selected) {
                ...
                //cout << "going" << endl;
                camobj.control(emily_location.x, emily_location.y,0.6*w/2,0.5*h/2);
                //cout<<"emily location : "<<emily_location.x<<","<<emily_location.y<<endl;
